...
import { deleteComment, fundingPickUpDetailApi, postComment } from '../api/api';
...
const FundingDetail = () => {
	...
	const { data } = useQuery<fashionPickupDetailData>(
		['get'],
		() => fundingPickUpDetailApi(url.pathname.split('/')[3]),
		{
							...
							data?.data.responseCommentDTOList.map((el) => {
								return (
									<CommentBoxDiv key={el.commentId}>
										...
										<CommentDiv>
											<span>{window.sessionStorage.getItem('displayName')}</span>
